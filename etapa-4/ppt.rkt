...
(define ppt-stream-in-tree-order
  (let helper ((Q (stream '(3 4 5))))
    (let ((t (stream-first Q)))
      (stream-cons t (helper (stream-append (stream-rest Q) (str_mul T1 t) (str_mul T2 t) (str_mul T3 t))))))) 
...
(define (pairs G H)
  (let helper ((GG G) (HH H) (lin 1) (col 1))
    (if (<= lin col)
        (stream-cons (cons (stream-first GG) (stream-first HH)) (helper (stream-rest GG) HH (+ lin 1) col))
        (helper G (stream-rest HH) 1 (+ col 1)))))
...
(define G
  (stream-filter odd? naturals-stream))

(define H
  (stream-rest G))

(define gh-pairs-stream
 (stream-filter (λ (pereche) (equal? (gcd (car pereche) (cdr pereche)) 1)) (pairs G H)))
...
(define ppt-stream-in-pair-order
  (let helper ((f gh-pairs-stream))
    (let ((first (stream-first f)))
      (let ((g (car first)) (h (cdr first)))
        (stream-cons (list (a g h) (b g h) (c g h)) (helper (stream-rest f)))))))
